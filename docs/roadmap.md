...
**Current verification gate (2026-06-26):** the active watch private-engine branch has deferred JVM/build/lint execution until the current implementation/review lanes finish. This Hermes environment cannot run Gradle because Java/Android tooling is unavailable. Treat previous `assembleDebug` / `compileDebugKotlin` / `testDebugUnitTest` green notes below as historical per-slice results, not current-branch verification. The authoritative current checklist is [`docs/watch-private-engine-test-plan.md`](watch-private-engine-test-plan.md).

## Watch / Active 4 Pro — direction changed (2026-06-24): vendored-SDK, hybrid

> **2026-06-26 execution goal.** Fabian wants the watch feature to work quickly for personal use without burning more time/tokens on byte-by-byte protocol rebuilding. Use the extracted VeryFit breakdown as an oracle for SDK workflows, but do **not** rebuild/rebrand the whole VeryFit app. Keep this Kotlin dashboard app as the product shell, keep the vendored IDO/VeryFit SDK quarantined behind `WatchEngine` / `IdoSdkWatchEngine`, and avoid VeryFit UI, ads, cloud, maps, membership, analytics, branding, and third-party telemetry. Working plan: [`docs/plans/veryfit-private-engine-workflow.md`](plans/veryfit-private-engine-workflow.md).
...
> earlier clean-room work remains historical protocol evidence in docs/git history/tag `pre-ido-sdk-baseline`. The active app tree now has a single SDK-backed watch path; a future independent engine would be recreated behind `WatchEngine` if/when needed.
...
> Verification labels in this table are historical per-slice notes from the session that produced that slice. They do not supersede the current 2026-06-26 deferred test plan for the active uncommitted branch.
