...
| W5 — Health upload: domain → DTO → `POST /api/widget/v1/watch/health` (+ dashboard route/schema) | 🔶 **in progress** — Android half done (DTOs + client + repository + `UploadingWatchHealthListener` + wiring; `compileDebugKotlin`/`testDebugUnitTest` green). Dashboard `POST /watch/health` route + workout unique-constraint authored on `schema/watch-health-metrics` (verify pending `npm install` + tsc). Idempotent upserts (day=user+date, workout=user+startedAt). Tables applied via `drizzle-kit push` (no migration file). |
